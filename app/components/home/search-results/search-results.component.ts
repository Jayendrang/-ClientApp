...
export class SearchResultsComponent implements OnInit {
  ...
  getStudentBooksList(flow){
    ...
    serviceParams.push({'key':'id','value':this.userDetails.user_info.user_id});
    this.restService.getData(WS.getBooksStudent.endpoint,(data) => {
      if(data && data.length>0 && !data.error){
        ...
        this.spinner.hide();
        this.toaster.openSnackBar(WS.getBooksStudent.errorMessage,'Close');
        this.books = [];
      }
      else{
        this.spinner.hide();
